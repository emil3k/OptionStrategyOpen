...
for i = 1:nMonths 
   day = RolloverDates(i);  %Grab rollover date
   ...
   startOfMonth = uniqueFirstDayList(i);
   endOfMonth = uniqueLastDayList(i);
   
   %Rf = RfDaily(startOfMonth:EndOfMonth);
   MonthLength = endOfMonth - startOfMonth;
   ...
   %cash2 = [1; cash];
        for j = 1:MonthLength
            ...
            elseif datesUnique(j) == day
                monthlyAccount(j) = monthlyAccount(j - 1) .* (1 + RfDaily(startOfMonth + j - 1)) + netDebit;
            elseif datesUnique(j) == expDate
                monthlyAccount(j) = monthlyAccount(j - 1) .* (1 + RfDaily(startOfMonth + j - 1)) - PL;        
            else
